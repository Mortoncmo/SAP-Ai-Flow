---
name: drawio-skill
description: Use when the user requests diagrams, flowcharts, architecture diagrams, ER diagrams, UML / sequence / class diagrams, network topology, ML/DL model figures (Transformer/CNN/LSTM), mind maps, or any visualization. Also use when revising an existing or user-edited .drawio source, resolving swimlane or connector overlap, or synchronizing the approved diagram into Word or PowerPoint. Best suited when the diagram needs custom styling, rich shape vocabulary, swimlanes, or exportable images (PNG/SVG/PDF/JPG). Generates and revises editable .drawio XML and exports locally via the native draw.io desktop CLI.
license: MIT
...
| `references/revision-and-office-sync.md` | The user has edited an existing `.drawio`, asks to fix swimlane/connector layout, or wants Word/PPT synchronized from the approved diagram without overwriting the editable source |
...
**Revision guard:** when an existing or user-edited `.drawio` is in scope, read `references/revision-and-office-sync.md` before any write or export. Treat the newest editable `.drawio` as the source of truth and do not run a generator path that rebuilds it unless the user explicitly requests regeneration.
