...
- 固定知识自动化评估集，覆盖 8 个检索案例和 4 个 GAP 正反例，并设置检索状态、Top-1 来源和 GAP 结果质量门槛。
...
- 持久化、修订、发布和导出接口的完整契约测试仍需扩展，尤其是具体身份提供方接入和跨环境回归场景；下载失败、核心权限矩阵、跨项目访问和发布预检测试已覆盖。
- 备份恢复和真实 PostgreSQL 环境迁移/故障注入验证尚未完成；Alembic 升级/回滚与事务写入故障注入已在 SQLite 回归测试中通过。
...
- 后端 `ruff check app tests alembic scripts/security_scan.py` 通过，pytest 65 项通过，包含成员迁移、成员角色门禁、项目知识路由、发布 GAP 审计、JWT 签名/claims、生产伪造身份、兼容接口关闭、Chroma 持久化/过滤/陈旧清理、固定知识/GAP 评估集、证据引用校验、连续 10 轮 ID 保留、外部模型策略、请求/异常日志、校验响应、脱敏红线、数据库事务故障回滚和依赖/导出失败保图。
...
- 前端 Vitest 27 项、TypeScript typecheck 和 production build 通过；覆盖成员 API、项目策略、导出文件名、导出失败错误、OIDC 配置、同源回调、防开放跳转、登录/退出回调和 Bearer Token 请求头。
- Alembic SQLite 升级/回滚、旧项目成员安全回填、DOCX 结构审计、表格 geometry、图片和标题层级审计通过。
- 独立 Playwright CLI 已完成管理员项目主流程、成员完整 CRUD、外部模型开关、`viewer` 身份及 Markdown / Word 下载验收：取消外发确认不改变设置，确认开启与直接关闭均成功并生成后端审计；中文下载文件名正确；1440 x 900、1024 x 768 和 390 x 844 显示正常，移动端页面与弹窗 `scrollWidth` 均等于 390；查看者可浏览，但编辑、泳道、布局、导入、自然语言指令和发布控件均禁用，控制台无 error。临时验收成员已清理。
- 最新泳道浏览器回归确认：点击“增加泳道”后只新增泳道且节点数保持 7；执行“增加泳道：销售、物流、财务”只补充不存在的泳道，已有财务不重复且节点数仍为 7；390 x 844 下 `body` 与根元素 `scrollWidth` 均为 390，控制台 0 error。
...
- 仓库已提供 `scripts/browser_smoke.ps1` 与 `scripts/browser_smoke.js`，可重复验证按钮/自然语言泳道操作、节点保持、390 x 844 宽度和控制台错误；成员 CRUD、权限矩阵、下载和跨环境主流程仍需继续扩展为仓库内自动化。
...
当前索引实现使用内容哈希生成确定性 chunk ID，ChromaDB 保存 chunk 文本、来源、模块、流程范围、SAP Release、审核/授权状态和嵌入向量。索引同步会删除已失效 chunk；模块、流程范围和 Release 在向量查询前执行元数据过滤。离线字符 n-gram 哈希嵌入用于可重复开发和私有化基线，不替代后续经固定评估集验证的生产语义模型。
...
- 右侧：节点基础属性、SAP 元数据、证据、GAP 和泳道管理。
...
3. 创建三个泳道时只增加泳道，不增加流程节点。
4. 编辑 SAP 元数据并查看证据。
5. 生成 GAP 候选，顾问确认和驳回。
6. 保存草稿、发布版本并以只读方式重新打开。
7. 导出 JSON、PNG、SVG、Markdown 和 Word。
8. 模型或知识服务失败时保留当前图。
9. 桌面与移动视口无控件遮挡和文本溢出。
...
- [x] 建立固定自动化检索评估集，覆盖 8 个正向、跨主题和证据不足案例。
...
- [x] 建立 4 个候选 GAP、标准能力和证据不足正反例自动化评估案例。
...
退出标准：可完整演示创建、修改、发布、回看和继续迭代；当前主流程已手工验收，仍需补充自动化契约和多视口回归。
...
- [ ] 自然语言和画布操作均可增改删节点、连线和泳道。
- [ ] 创建多个泳道不会误创建流程节点。
- [ ] 连续 10 轮修改不会误改未涉及对象。
- [ ] 撤销、重做、自动布局和本地恢复正常。
- [ ] 1.0 与 2.0 JSON 均可安全导入。

### SAP 蓝图

- [ ] MM/P2P 节点可维护 T-Code、Fiori App、角色、配置点和 Best Practice 引用。
- [ ] AI 建议均可查看证据或待确认状态。
- [ ] GAP 只能由顾问确认。
- [ ] 项目、流程、修订和发布版本可完整追溯。
- [ ] Markdown 与 Word 蓝图内容一致。
...
- [ ] Docker Compose 可在干净环境启动全部 V1.0 服务。

## 18. 下一开发切片

基础契约、持久化、版本工作流、服务端项目成员角色、项目级知识/GAP 查询、发布 GAP 审计和同步文档导出已经落地。后续按风险优先级推进：
...
3. 在现有泳道/移动端 Playwright smoke 基础上，继续固化成员 CRUD、外部模型策略、权限矩阵、Markdown / Word 下载和多视口验收，并在具备 LibreOffice 的环境完成 DOCX 视觉渲染。
4. 更新 README 和部署说明，在具备 Docker 与 LibreOffice 的环境完成 PostgreSQL、Compose、中文字体、迁移、备份恢复和文档渲染验收。
5. Tool 契约、真实认证和向量检索稳定后再接入 LangGraph；Python SDK、CLI 和 BPMN 保持后续优先级。

外部模型数据策略、知识证据门禁、文档浏览器下载、通用 SPA PKCE、应用日志治理和依赖/秘密扫描已完成代码与自动化测试。当前最优先项转为具体身份提供方、集中日志和部署联调；在缺少外部环境时，优先固化 Playwright 回归并推进备份恢复、性能和故障注入检查。
