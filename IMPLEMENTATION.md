...
> 文档状态：新版 Markdown 逐章整合、OIDC Token 生命周期与生产边界基线 4.13
>
...
>
> 配套文档：用户操作见 [USER_MANUAL.md](USER_MANUAL.md)，生产部署与维护见 [deploy/OPERATIONS.md](deploy/OPERATIONS.md)
>
...
本追踪表之外，新版说明未定义但生产交付必须补齐的门槛包括：OIDC 真实身份提供方客户端联调、知识来源授权、日志脱敏与集中告警、真实外部模型/PostgreSQL 容量、缓存多实例策略以及目标环境部署与运维签字。项目租户归属、OIDC 租户 claim allowlist 和跨租户资源隐藏已在仓库内实现，但仍需用目标 IdP 的真实 claim 完成联调。这些项目统一在第 15 至 18 节跟踪，未关闭前不得宣称生产验收完成。
...
- `ProjectMember` 服务端成员模型、Alembic 迁移和成员管理 API；项目创建者自动成为 `project_admin`，项目角色从服务端成员记录解析，成员新增、角色变更和移除与 `ProjectAuditRecord` 在同一事务提交。
- 项目租户隔离：项目记录固化 `tenant_id`，生产 JWT 从可配置 `OIDC_TENANT_ID_CLAIM` 读取租户并命中 `OIDC_ALLOWED_TENANT_IDS` allowlist；项目/流程/知识授权先校验租户，跨租户资源统一隐藏为 404，0009 迁移为存量项目回填 `local`。
...
- 前端 OIDC Authorization Code + PKCE 登录、回调 URL 清理、退出、`sessionStorage` 会话和全 API Bearer Token 注入；IdP 签发 Refresh Token 时在 Access Token 最后 60 秒单飞续期，未签发或续期失败时在到期后清会话，受保护 API 返回 401 时也同步切回登录 UI，且不自动重放写请求；未配置 OIDC 时保持本地开发模式，配置不完整时阻断并显示明确错误。
- Compose 增加只在 `oidc-acceptance` profile 启动的标准测试 IdP；协议级浏览器门禁覆盖 Authorization Code、PKCE S256、Token 请求 `code_verifier`、Bearer Token 调用受保护项目 API、创建者角色、tenant claim 持久化、退出和匿名 401，并要求失败日志和验收报告不暴露 code、state 或 Token。GitHub Actions 运行 `31395094356` 已取得四作业全绿和脱敏验收报告，仓库内通用 OIDC 协议链路标记为通过。
...
- 生产身份上下文同时携带 `user_id` 和 `tenant_id`；开发环境固定为 `DEVELOPMENT_TENANT_ID`，不把客户端租户头当作可信来源。
...
- GitHub Actions 独立 `deploy` 作业已固化 PostgreSQL/API/Web、两个健康 Worker、readiness 中的 `artifact_storage_status=ready`、Alembic 0009、12 个普通任务恰好执行一次、文件系统对象不写入数据库、1 个陈旧租约任务只接管一次、心跳新鲜任务不被接管且旧 Token 写回被拒绝，再执行 `pg_dump`、恢复到独立数据库并核对迁移版本。
...
- 长文档 Worker 中断验收：构造 80 节点 DOCX，等待首个 Worker 心跳推进后强制终止容器，经过真实陈旧窗口再由替代 Worker 接管；最终要求 `attempt_count=2`、有效 DOCX、外部交付物完整且数据库二进制为空。受控渲染延迟只允许 acceptance 模式，生产配置会拒绝启动。
...
- 具体 SSO/OIDC 身份提供方的客户端注册、真实登录/退出、Refresh Token 签发/轮换或到期重新登录策略、真实 tenant claim 和存量项目映射联调尚未完成；通用 SPA PKCE、Bearer Token、Refresh Token 条件续期、过期清会话、租户 allowlist、项目租户归属和跨租户 404 隔离已完成，`X-User-ID` 仅保留在开发环境。标准测试 IdP 只能验证通用协议链路，不能替代目标企业身份策略和存量项目映射签字。
...
- 独立 Worker 已在本地 SQLite 双进程回归和 GitHub PostgreSQL 双 Worker 场景完成领取、心跳续租、并发恰好一次、心跳新鲜任务保留和陈旧租约接管；仓库还固化 80 节点 DOCX 的进程强杀、真实陈旧等待和新 Worker 接管门禁。目标最大文档规模、实际资源上限及部署控制器滚动中断仍需目标环境验证。
...
- 当前权限实现已从服务端 `ProjectMemberRecord` 解析项目角色，不再信任客户端 `X-Project-Role`；成员新增、角色变更和移除已记录前后值、操作者和时间，可由管理员在界面中查询。开发环境缺省身份为 `local-user`，生产环境只接受通过 OIDC/JWKS 校验的 Bearer JWT。前端已具备 SPA PKCE 登录、回调、退出和 Token 注入，Compose/CI 的标准测试 IdP 协议门禁已全绿；具体身份提供方客户端注册及部署联调完成前，仍不能视为完整生产认证方案。
...
- 当前滚动中断门禁不通过伪造陈旧时间替代进程故障：首个 Worker 必须已领取 80 节点 DOCX 且至少续租一次，随后容器被强制终止；替代 Worker 只能在 1 分钟租约真实过期后领取，并必须生成可打开的 DOCX。目标环境仍需用实际最大交付物和部署控制器复跑。

### 3.4 本轮验证记录（2026-08-10）

- 后端 `ruff check .` 通过，pytest 130 项通过，新增覆盖 80 节点长文档夹具、production 禁用验收延迟和会话级临时数据库/Chroma 隔离，并继续覆盖目标 S3 验收探针、Alertmanager 接收器字段白名单、成员审计、OIDC 租户隔离、Prometheus 低基数、文件系统/S3 完整性、租约与陈旧接管、LangGraph 路由、证据不足、文档导出、迁移、容量契约、RAG/GAP、权限、连线原子操作、Provider 时限/缓存、JWT、脱敏、事务回滚和依赖失败保图。
...
- 前端 Vitest 增加 OIDC Token 生命周期回归；除既有异步导出、成员/审计、OIDC 配置、同源回调、防开放跳转、登录/退出和 Bearer Token 外，还覆盖 Refresh Token 临近过期续期、并发续期合并、无 Refresh Token 的过期清理、401 会话失效且写请求不重放和应用登录态同步。TypeScript typecheck 和 production build 继续作为 CI 门禁。
- Alembic SQLite 升级/回滚（含 0007 `heartbeat_at` 回填、0008 交付物字段和索引、0009 项目租户回填）、`export_job` 迁移、旧项目成员安全回填、DOCX 结构审计、表格 geometry、横竖分节、表格行禁拆、图片和标题层级审计通过。
...
- GitHub Actions 运行 `31395094356` 对提交 `5b66fc8` 的 API、Web、`docx-render` 和 `deploy` 四个作业全部通过。标准测试 IdP 使用身份 `ci-consultant`、租户 `ci-tenant` 和 audience `sap-ai-flow-ci`；脱敏 `oidc-acceptance.json` 确认 `authorization_code_pkce=true`、`token_exchange=true`、`protected_api_bearer=true`、`tenant_claim_persisted=true`、`logout=true`、`anonymous_api_rejected=true`。`deploy` 同时通过 0009、Alertmanager 回调、双 Worker 栅栏、80 节点 DOCX 中断恢复和 PostgreSQL 备份恢复。本证据关闭仓库内标准 IdP 通用协议验收，不替代目标企业 IdP、真实 Token 生命周期和存量项目映射联调。
- GitHub Actions 运行 `31399687656` 对提交 `257525b` 的 API、Web、`docx-render` 和 `deploy` 四个作业全部通过。Web 作业执行 37 项测试，覆盖 Refresh Token 条件续期、并发续期合并、过期/401 会话清理、登出竞态和写请求不重放；deploy 再次取得标准测试 IdP 全通过报告，readiness 确认 OIDC tenant allowlist、PostgreSQL、独立 Worker 和 filesystem 交付物就绪，80 节点中断恢复继续返回 `interrupted_attempt_count=2`、`replacement_completed=true`、`database_content_empty=true`，Alembic 为 `20260810_0009 (head)`。真实 IdP 的 Refresh Token 签发/轮换或到期重登策略仍需目标环境联调。
- `pip-audit` 在升级 Pillow 12.3.0 后无可修复漏洞；ChromaDB `PYSEC-2026-311`、`CVE-2026-45830`、`CVE-2026-45831` 和 `CVE-2026-45833` 因项目未暴露 Chroma HTTP API、未配置其认证/RBAC Provider、未开放模型仓库配置而按 `SECURITY.md` 限定例外，并设 2026-09-09 复核日。`npm audit --omit=dev` 为 0 漏洞，生产源码/前端构建秘密扫描通过。
...
GET    /api/v1/projects/{project_id}/audits          # 仅 project_admin 查看项目策略与成员变更审计
...
- 成员新增、角色变更、移除和外部模型策略变更均在“审计”标签显示操作、对象、前后状态、操作者和时间。
- 移动端沿用成员管理底部弹窗布局，成员与审计标签必须在 390 x 844 视口无横向溢出。
...
- 成员新增、角色变更和移除与业务写入同事务记录到 `ProjectAuditRecord`；无变化更新不产生重复审计，只有 `project_admin` 可通过 API 和前端标签查看。
...
- 标准测试 IdP 的 Compose profile 和协议验收脚本已通过 GitHub Actions 运行 `31395094356`，验证通用 SPA PKCE 登录、Token 交换、Bearer API、tenant 持久化、退出和匿名拒绝链路；脱敏报告随 `deployment-acceptance` 构件归档。前端另支持 IdP 实际签发 Refresh Token 时的条件续期，并在无续期能力或最终续期失败后清除过期会话、同步 UI 且不自动重放写请求。
...
- Alembic 0006 增加导出领取 Token、尝试次数和候选索引；0007 增加 `heartbeat_at` 及心跳候选索引，并把存量运行任务的心跳回填为 `started_at`；0008 增加 `artifact_backend`、`artifact_key`、`content_sha256` 和过期清理索引，完成/现存数据库二进制按兼容规则标记为 `database`；0009 增加项目 `tenant_id` 与索引并将存量项目回填为 `local`，生产升级时必须在开放流量前映射到真实 IdP 租户；升级后旧任务 `attempt_count=0`，回滚各迁移移除对应列和索引。
...
- [x] 以 0009 增加项目 `tenant_id` 与索引、存量项目 `local` 回填和 SQLite 开发库兼容升级，并在运维文档固化生产 tenant ID 映射门禁。
...
- [x] 完成 Refresh Token 条件续期、并发续期合并、Access Token 到期清会话和 UI 同步；目标 IdP 是否签发/轮换 Refresh Token 或采用到期重登仍由环境联调确认。
- [x] 接入标准测试 IdP、`oidc-acceptance` Compose profile 和协议级 Playwright 门禁，覆盖 PKCE、Token 交换、Bearer、tenant、角色、退出、匿名拒绝和脱敏报告。
- [x] 修复标准测试 IdP 验收脚本的 Playwright CLI 兼容和对话框时序问题；运行 `31395094356` 的 API、Web、DOCX 和 deploy 全绿，脱敏 OIDC 报告已归档。
...
- [x] 固化 80 节点 DOCX 的执行中容器强杀、真实陈旧窗口等待、替代 Worker 第 2 次尝试完成和交付物完整性门禁。
...
- [x] 完成 80 节点 DOCX 的进程级滚动中断恢复：首个 Worker 续租后被强制终止，替代 Worker 在租约真实过期后完成第 2 次尝试，验收延迟在生产模式被禁止。
...
- [x] 修复标准测试 IdP 浏览器验收脚本兼容问题，并由运行 `31395094356` 取得 deploy 作业和脱敏协议报告通过证据。
- [x] 补充面向 SAP 顾问、流程编辑者、审批者、项目管理员和查看者的中文操作手册，覆盖独立泳道管理、节点图标、权限、修订、GAP、发布、导出和错误恢复。
...
- README、用户操作手册、部署配置、示例和本实施文档同步更新。
...
- [x] 标准测试 IdP 的协议级浏览器门禁在 GitHub Actions 运行 `31395094356` 全绿，验收报告包含 PKCE、Token 交换、Bearer、tenant、退出和匿名拒绝结果。
- [ ] 目标企业 IdP 已完成客户端注册、真实登录/退出、Refresh Token 签发/轮换或到期重登、tenant claim 和存量项目映射联调。
...
1. 按 `deploy/OPERATIONS.md` 在目标 SSO/OIDC 身份提供方登记 SPA 客户端、回调/退出地址和 API audience，完成真实登录、退出、Refresh Token 签发/轮换或到期重登、tenant claim 与存量项目映射联调。仓库已完成 Refresh Token 条件续期、过期清会话和 UI 同步，标准测试 IdP 的 Authorization Code + PKCE、Token 交换、Bearer、tenant 持久化、退出和匿名拒绝门禁已全绿并归档脱敏报告。
...
当前本机可执行的最终流程编辑验收项、OIDC 租户 claim/allowlist 与跨租户隔离、标准测试 IdP 协议门禁、受控交付物存储单元/SQLite 回归、目标 S3 验收工具、独立 Worker 双进程验收、心跳续租与租约丢失补偿回归、80 节点 DOCX 进程强杀/真实陈旧恢复门禁、GitHub Actions 0009 Compose/PostgreSQL 双 Worker 共享交付物验收，以及 LibreOffice/Noto CJK DOCX 跨渲染器验收基线已完成。生产验收剩余门槛还包括具体身份提供方真实登录、Token 生命周期和 tenant claim/存量项目映射联调、知识授权与顾问签字、集中日志、真实外部模型容量、目标最大文档/部署控制器滚动中断、企业 S3/文档库实跑及非当前版本清理/备份恢复、目标环境 DOCX 复跑和部署联调。这些项目未验证前不得宣称生产验收完成。
