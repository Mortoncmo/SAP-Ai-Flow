...
- 可自行分配端口并启动当前 API、Web 和隔离 SQLite 数据库的 Playwright CLI smoke；覆盖泳道不误增节点、成员 CRUD、外部模型二次确认与审计、持久化修改/发布/历史回看/新草稿、管理员/查看者权限、Markdown/Word 下载和三种视口。
...
- 后端 `ruff check app tests alembic ../../scripts/security_scan.py` 通过，pytest 63 项通过，包含成员迁移、成员角色门禁、项目知识路由、发布 GAP 审计、JWT 签名/claims、生产伪造身份、兼容接口关闭、Chroma 持久化/过滤/陈旧清理、固定知识/GAP 评估集、证据引用校验、连续 10 轮 ID 保留、外部模型策略、请求/异常日志、校验响应、脱敏红线、数据库事务故障回滚和依赖/导出失败保图。
...
- 仓库内 Playwright CLI smoke 默认自动分配端口并启动当前 API、Web 和隔离 SQLite 测试库，不依赖已运行的开发进程。它已完成持久化自然语言修改、发布只读、从发布版本创建新草稿、发布历史只读回看和返回当前草稿的生命周期，并覆盖成员完整 CRUD、外部模型开关、`viewer` 身份及 Markdown / Word 下载：取消外发确认不改变设置，确认开启与直接关闭均成功并新增两条后端审计；中文下载文件名和 Markdown 内容正确，DOCX 具有有效 ZIP 文件头；查看者可浏览和导出，但成员、策略、编辑、泳道、布局、导入、自然语言指令和发布能力均受限。
- 同一 smoke 确认点击“增加泳道”后只新增泳道且节点数保持 7；执行“增加泳道：销售、物流、财务”只补充不存在的泳道，已有财务不重复且节点数仍为 7；1440 x 900、1024 x 768 和 390 x 844 的页面宽度正常，移动端成员弹窗无横向溢出，控制台与页面均为 0 error。
...
- `scripts/browser_smoke.ps1` 与 `scripts/browser_smoke.js` 已固化泳道、成员 CRUD、外部模型策略、持久化发布生命周期、管理员/查看者权限、实际下载和三视口回归；还需继续扩展前端故障重试、超时和性能回归。
...
- [x] 完成泳道、成员、外部模型策略、持久化发布生命周期、查看者权限、蓝图下载和三视口的仓库内 Playwright 验收。
- [ ] 完成 Docker 和部署验收。
- [ ] 完成数据库迁移、备份恢复和故障排查文档。
...
3. 在现有自包含 Playwright smoke 基础上，继续固化前端故障重试、超时和性能回归，并在具备 LibreOffice 的环境完成 DOCX 视觉渲染。
...
外部模型数据策略、知识证据门禁、文档浏览器下载、通用 SPA PKCE、应用日志治理、依赖/秘密扫描和持久化发布/权限/下载 Playwright smoke 已完成代码与自动化测试。当前最优先项转为具体身份提供方、集中日志和部署联调；在缺少外部环境时，优先补齐故障重试与性能回归并推进备份恢复和故障注入检查。
