...
- 后端 `ruff check app tests alembic ../../scripts/security_scan.py` 通过，pytest 69 项通过，包含 DeepSeek 超时/总时限、429/5xx 重试、认证不重试、非法输出分类，以及成员迁移、成员角色门禁、项目知识路由、发布 GAP 审计、JWT 签名/claims、生产伪造身份、兼容接口关闭、Chroma 持久化/过滤/陈旧清理、固定知识/GAP 评估集、证据引用校验、连续 10 轮 ID 保留、外部模型策略、请求/异常日志、校验响应、脱敏红线、数据库事务故障回滚和依赖/导出失败保图。
...
- 同一 smoke 通过方向切换和自动布局产生两条历史，连续撤销/重做后版本与方向正确，刷新后当前图、版本、7 个节点和 5 条泳道恢复；撤销栈按设计不跨刷新保存。点击“增加泳道”后只新增泳道且节点数保持 7；执行“增加泳道：销售、物流、财务”只补充不存在的泳道，已有财务不重复且节点数仍为 7；主动取消、2 秒测试超时和模拟 409 后节点数与修订保持不变、指令保留，超时后重试成功；20 次本地 Patch 采样 P95 小于 1 秒；1440 x 900、1024 x 768 和 390 x 844 页面宽度正常，移动端成员弹窗无横向溢出，控制台与页面均为 0 error。
...
- `scripts/browser_smoke.ps1` 与 `scripts/browser_smoke.js` 已固化本地历史/恢复、泳道、成员 CRUD、外部模型策略、取消/超时/冲突恢复、本地 P95、持久化发布生命周期、管理员/查看者权限、实际下载和三视口回归；还需在真实外部模型/PostgreSQL 环境扩展并发、长尾与容量测试。
...
8. 模型或知识服务失败、用户取消、客户端超时或修订冲突时保留当前图与修订。
9. 桌面与移动视口无控件遮挡和文本溢出。
...
外部模型数据策略、知识证据门禁、文档浏览器下载、通用 SPA PKCE、应用日志治理、依赖/秘密扫描、Provider 时限/重试和本地历史/恢复/持久化发布/故障恢复/权限/下载 Playwright smoke 已完成代码与自动化测试。当前最优先项转为具体身份提供方、集中日志、真实 PostgreSQL/外部模型、LibreOffice 和部署联调。
